...
import { computed, effect, ObservableObject, observableObject, Signal, signal } from "@tui/signals";
import { OverlayBlock, Style, StyleBlock } from "@tui/nice";
...
// FIXME: scroll text when oob
export function createTextBox(styles: Record<TextBoxClass | "cursor", Style>) {
    const getState = tui.createLocalStates<TextBoxState>((placeholder) => {
        // TODO: Observable arrays?
        const lines = signal([""]);
...
export const TextBox = createTextBox({ base, hover, active, cursor });
